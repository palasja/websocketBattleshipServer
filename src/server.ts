    ...
    ws.on('message', function message(mes) {
      ...
      switch (messageReq.type) {
        ...
        case RequestType.SINGLE_PLAY: {
          const playerRoom = availableRooms.find((r) =>
            r.roomUsers.find((p) => p.index == playerId)
          );
          if (playerRoom) {
        ...
        case RequestType.ADD_USER_TO_ROOM: {
          ...
          games.push(game);
          const playerRoomn = availableRooms.find((r) =>
            r.roomUsers.find((u) => u.index == playerId)
          ) as Room;
        ...
        case RequestType.ADD_SHIP: {
          ...
          const curPlayerInfo = curGame?.players.find(
            (p) => p.sessionId == shipsInfo.indexPlayer
          ) as PlayerGameInfo;
