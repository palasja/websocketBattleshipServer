...
const getGamePlayers = (
  roomOwnerPlayerId: string | number,
  secondPlayerId: string | number
): PlayerGameInfo[] => {
  const player1Socket = sockets.find((s) => s.playerId == roomOwnerPlayerId)
    ?.ws as unknown as WebSocket;
  const player1Name = players.find((p) => p.id == roomOwnerPlayerId)?.name as string;
  const player2Socket = sockets.find((s) => s.playerId == secondPlayerId)
    ?.ws as unknown as WebSocket;
...
export { newPlayer, createRoom, getGamePlayers };
